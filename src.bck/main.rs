...
use scanner::{
    ArbTransaction, ScannerContext, cli::Cli, config::ScannerConfig, dashboard::Dashboard, grpc::Grpc, process::Process
};
...
async fn main() -> anyhow::Result<()> {
    ...
    let config = ScannerConfig::from_toml_file(cli.config)?;
    ...
    info!("Scanner started with {} endpoints", config.grpc.endpoints.len());
