...
use yellowstone_grpc_proto::geyser::{CommitmentLevel, SubscribeRequest, SubscribeRequestFilterTransactions, SubscribeRequestPing, subscribe_update::UpdateOneof};
...
impl MultiGrpc {
        ...
        F: Fn(usize, ConnectionStatus, u64) + Send + Sync + 'static,
    {
        ...
        status_cb(idx, ConnectionStatus::Connected, 50);
    ...
    // pub async fn ping(&self) -> Result<Vec<u64>> {
    //     let sinks = self.sinks.lock().await;
    //     let mut request = self.request.lock().await.clone();
    //     let mut ping_sent_at = self.ping_sent_at.lock().await;

    //     request.ping = Some(SubscribeRequestPing { id: 123 });
    ...
    //                     ping_sent_at[idx] = Some(now);
    //                     debug!("Sent ping to endpoint {}", idx);
    //                 }
    //                 Err(e) => {
    //                     debug!("Failed to send ping to endpoint {}: {:?}", idx, e);
    //                 }
    //             }
    //         } else {
    //             debug!("Endpoint {} not connected, skipping ping", idx);
    //         }
    //     }

    //     // Return current roundtrip times
    //     let roundtrips = self.ping_roundtrip.lock().await.clone();
    //     Ok(roundtrips)
    // }
